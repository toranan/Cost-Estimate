...
## 위원회 비용추계 라우팅 엔진 (TAG 룰엔진)

법안이 만드는 **위원회·조직 비용**을 계산하는 규칙 기반 엔진(`backend/tag_pipeline.py`)의
구조입니다. 핵심 설계는 **"비용 = 산식(계산 구조) × 전제값들"** 로 보고, 산식은 조문에서
판단하되 **전제값을 4단계로 조달**하는 것입니다.

### 전제값 조달 4단계

```
비용항목
 ① 조문에 값이 있나        → 위원 정수·증원 인원 (명시되면 확정)
 ② 규칙 라우팅으로 계산되나  → 회의수당·상임위원 증원·기본경비율 (규칙 게이트 + 계산기)
 ③ DB에서 개념으로 조회되나  → 급여·기관부담률·단가 (method2: 개념 키 → 선례 population 대표값)
 ④ 그래도 없으면           → HITL (산식 제시 + 빈 전제만 콕 집어 입력 요청, 입력 시 즉시 계산)
```

### 라우팅 → 산식 (카테고리별 계산기)

Phase 1 라우터(`tag_router.py`)가 조문을 **6대 카테고리**로 분류하고, Phase 3
룰엔진(`tag_rule_engine.py`)이 유형별 산식으로 계산합니다.

| 카테고리 / 유형 | 산식 | 전제값 출처 |
|---|---|---|
| **위원회 회의수당** | `(위원 정수 − 당연직) × 회의수당단가 × 회의횟수` | 위원수=조문 / 단가=표준 25만·기관조회 / 횟수=조문 or 폴백 연 4회 |
| **상임위원 인건비** | `상임 인원 × 급여 × (1+기관부담률+기본경비율)` | 급여=TAG DB 동적조회(등급미상) or 봉급표 |
| **개정형 상임위원 증원** | `증원 인원 × 정무직급여 × (1+기관부담률+기본경비율)` | 정무직급여 1.50억(차관급) / "겸직 배제 = 인원 +1" 일반 규칙 |
| **일반직 정원 증원** | `증원 정원 × 급여 × (1+기관부담률+기본경비율)` | 급여=직급표 or 전체공무원 평균 기준소득월액(직급 미상 시) |
| **유형1 중앙기관** | `정무직 위원장 연봉 + 소속공무원 인건비 + 청사 건축비` | 인원 없으면 위원장 연봉만 확정, 나머지 보류 |
| **유형2 사무국** | `회의수당 + 상임위원 인건비 + 사무국 인건비 + 임차료` | 사무국 인원 없으면 확정분만, 나머지 보류 |

**공통 계산 상수** — 기관부담률 `0.13066`(공무원 신규채용 기관부담요율, 공표),
기본경비율 `0.116`(총인건비 대비, TAG DB 실측 중앙값), 회의횟수 폴백 `4회`(DB 최빈값).

### 라우팅 게이트 (조문 → 유형 판정)

- `committee_gate.py` — 흩어진 설치·구성·운영 조문을 **개체 단위**로 그룹핑(중복계산 방지)
- `committee_type_gate.py` — 유형1(중앙기관)·유형2(사무국)·유형3(단순자문)·유형4(국회자체) 판정
- `committee_increment_gate.py` — 개정형 "N→M 증원", "겸직 배제 +1" 탐지 (규칙 + LLM 백업)
- `committee_temporality.py` — 한시/반복무한("때마다") 판정 → 반복무한은 보류
- `committee_replication.py` — 시·도별 복제설치 배수 반영

### 동적 참조 조회 (method2)

`tag_reference_lookup.py` — 계산 상수를 하드코딩 대신 **TAG DB(공식 추계서 구조화본)에서
개념 키로 조회**합니다. "가장 비슷한 선례 하나"가 아니라 **개념 population의 대표값**을
쓰며(유사도-단일선택의 오탐 회피), 좁게 수렴할 때만(표본 ≥3, 최대/최소 ≤1.5) 확정값으로,
넓으면 저신뢰로 라벨합니다.

- 예: 소속기관 "법무부" → 기본경비율 `9.6%`(실측 정답과 일치), 연도축 → 기관부담률

### 신뢰도 라벨 (정답 없이 결정)

처음 보는 의안은 오차 자체를 알 수 없지만, **오차가 클 조건은 정답 없이 판단**합니다 —
전제를 조문/DB에서 채웠는지, 분포가 수렴하는지로 `🟢확정 / 🟡저신뢰 / 🔴보류`를 매깁니다.
개발에 쓰지 않은 새 의안 10건 홀드아웃에서 확정 계산 6건 전부 오차 30% 이내(4건은 5% 이내),
신뢰도 라벨이 실제 오차 방향과 일치했습니다. → [상세](./포트폴리오/CASE_STUDY_ROUTING_ENGINE.md)

## 문서

- [상세 아키텍처](./ARCHITECTURE.md)
- [라우팅 엔진 케이스 스터디(전제조달·산식·홀드아웃 10건)](./포트폴리오/CASE_STUDY_ROUTING_ENGINE.md)
- [홀드아웃 10건 케이스별 상세](./포트폴리오/HOLDOUT10_RESULTS.md)
