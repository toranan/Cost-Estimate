...
- 확정된 변수와 계산 조건이 같으면 같은 산술 결과

확정된 변수에 대한 산술을 재현 가능하게 만드는 것이 이 분리의 목적입니다. 상위 단계의 LLM 해석, 검색 결과, 외부 통계가 달라질 수 있으므로 전체 파이프라인이 항상 같은 결과를 낸다는 의미는 아닙니다.
...
### ✅ 보장
- **결정적 계산** — 확정된 변수와 계산 조건이 같으면 같은 산술 결과
